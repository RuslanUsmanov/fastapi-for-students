...
8. Для того чтобы исключить эти функции из подсчета покрытия мы добавим к ним комментарий `# pragma: no cover`:
...
```
9. Снова запустим тесты с помощью `coverage`, чтобы перегенерировать отчет:
```bash
$ coverage run -m pytest tests
```
10. Проверяем отчет:
...
1. Написать тесты к своим API, которые были реализованы в предыдущей лабораторной работе.
2. Обеспечить 100% покрытие кода, для проверки покрытия использовать библиотеку `coverage`.
3. Результаты работы выложить в свой репозиторий на GitHub или GitLab.
